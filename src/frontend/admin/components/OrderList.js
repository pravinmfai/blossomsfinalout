...
const OrderList = () => {
  ...
  const [showResolveButton, setShowResolveButton] = useState({}); // Track if "Resolved" button is shown

  const apiUrl = process.env.REACT_APP_API_URL; // Use the base API URL from env
  const ordersApi = `${apiUrl}/allorders`;
  const searchApi = `${apiUrl}/orders/search`;

  const fetchProducts = useCallback(async (orders) => {
    try {
      const productPromises = orders.map(order =>
        axios.get(`${apiUrl}/orders/${order._id}`) // Use the base API URL from env
      );
  ...
  const fetchOrders = useCallback(async (query = '') => {
    setLoading(true);
    try {
      const url = query ? `${searchApi}?query=${query}` : ordersApi;
  ...
  }, [ordersApi, searchApi, fetchProducts]);
  ...
  const handleConfirmShipping = async (orderId) => {
    ...
    try {
      await axios.put(`${apiUrl}/orders/${orderId}/shipping`, { // Use the base API URL from env
  ...
  const handleResolveIssue = async (orderId) => {
    try {
      await axios.put(`${apiUrl}/orders/${orderId}/shipping`, {
        shippingStatus: 'Shipped'
      });
      toast.success('Order marked as resolved and status updated to Shipped');
      fetchOrders();
    } catch (error) {
  ...
  const handleCancelResolve = (orderId) => {
    setShowResolveButton(prevState => ({ ...prevState, [orderId]: false })); // Hide "Resolved" button
  };
            ...
            return (
              <div key={order._id} className="order-card">
                ...
                <div className="order-column right-column">
                  {showShippingForm[order._id] ? (
                  ...
                  ) : order.primaryInfo.shippingStatus === 'Issues' && showResolveButton[order._id] ? (
                    <div className="resolve-actions">
                      <button
                        className="button resolve-button"
                        style={{ backgroundColor: 'lightgreen', color: 'white' }} // Green "Resolved" button
                        onClick={() => handleResolveIssue(order._id)}
                      >
                        Resolved
                      </button>
                      <button
                        className="button cancel-button"
                        onClick={() => handleCancelResolve(order._id)}
                      >
                        Cancel
                      </button>
                    </div>
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        } else if (order.primaryInfo.shippingStatus === 'Issues') {
                          setShowResolveButton(prevState => ({ ...prevState, [order._id]: true }));
                        }
                      }}
                    >
